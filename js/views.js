...
async function viewPlayerStats(playerSelected, currentRaid, allSeasonRaids) {
    ...
    if (tokens.tokens) {
        ...
        <table class="tokensTable">
          <caption>Tokens (<span class="${tsNow > tokens.lastUpdatedOn/timeStampDivisor+2*60*60 ? "warning" : ""}">${toDateStr(tokens.lastUpdatedOn)}</span>)</caption>
...
function viewActivity(currentRaid, allSeasonRaids, guildData) {
    ...
    let rows = guildData.guild.members.filter(member => activityFilterBombs ? hasBomb[member.userName] : true).map(member => {
        ...
        entries.forEach(e => {
            ...
            hour = +(new Date(e.completedOn).toLocaleString("en-US", {hour: '2-digit', hour12: false, timeZone: systemTimeZone}));
            activity[hour]++;
            activityTotal[hour]++;
        });
